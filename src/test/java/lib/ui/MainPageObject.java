...
public class MainPageObject {
    ...
    public Boolean waitTextPresentInElement(By by, String expected_text, String error_message, long timeoutInSeconds) {
        WebDriverWait wait = new WebDriverWait(driver, timeoutInSeconds);
        wait.withMessage(error_message + "\n");
    ...
    protected List<WebElement> waitForElementsPresent(String locator, String errorMessage, long timeOutInSeconds) {
        By by = this.getLocatorByString(locator);
        WebDriverWait wait = new WebDriverWait(driver, timeOutInSeconds);
    ...
    protected boolean isElementLocatedOnTheScreen(String locator) {
        int location_by_y = this.waitForElementPresent(locator, "Can't find element by locator", 1).getLocation().getY();

        if(Platform.getInstance().isMW()){
            JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
            Object js_result = jsExecutor.executeScript("return window.pageYOffset");
            location_by_y -= Integer.parseInt(js_result.toString());
        }

        int screen_size_by_y = this.driver.manage().window().getSize().getHeight();
        return location_by_y < screen_size_by_y;
    }
